...
function BandwidthScorecard() {
  ...
  if (!data?.summary)
    return <p className="text-small text-text-muted italic">Loading...</p>;
  const { summary } = data;
  ...
  return (
    <div className="space-y-3">
      {hasUtil && summary.over_utilised.length > 0 && (
        <div>
          <p className="text-data text-sm text-red mb-1">
            Over-utilised (&gt;80%)
          </p>
          {summary.over_utilised
            .slice(0, 5)
            .map((s: { site: string; pct: number; provider: string }) => (
              <div
                key={s.site}
                className="flex justify-between text-data text-sm"
              >
                <span className="text-text-primary">{s.site}</span>
                <span>
                  <span className="text-red">{s.pct}%</span>{' '}
      ...
      {hasUtil && summary.under_utilised.length > 0 && (
        <div>
          <p className="text-data text-sm text-yellow mb-1">
            Under-utilised (&lt;10%)
          </p>
          {summary.under_utilised
            .slice(0, 5)
            .map((s: { site: string; pct: number; provider: string }) => (
              <div
                key={s.site}
              ...
              >
                <span className="text-text-primary">{s.site}</span>
                <span>
                  <span className="text-yellow">{s.pct}%</span>{' '}
                  <span className="text-text-muted">{s.provider}</span>
                </span>
              </div>
            ))}
      ...
      {summary.zero_traffic.length > 0 && (
        <p className="text-data text-sm text-orange">
          {summary.zero_traffic.length} sites with near-zero traffic
        </p>
      )}
      <p className="text-small text-text-muted">
        Utilisation data for {summary.sites_with_utilisation} of{' '}
        {summary.sites_with_bandwidth} sites.
        {!hasUtil &&
          ' Coverage improves as NetBox circuit records are completed.'}
      </p>
    </div>
  );
}
...
function CoherenceCard() {
  ...
  const items = [
    {
      label: `${data.netbox_coverage.matched} / ${data.netbox_coverage.total_grafana} sites in NetBox`,
      pct: data.netbox_coverage.pct,
    },
    {
      label: `${data.commit_rate_coverage.with_rate} / ${data.commit_rate_coverage.total_circuits} circuits with committed rate`,
      pct: data.commit_rate_coverage.pct,
    },
  ];
    ...
    <div className="space-y-3">
      ...
      {data.grafana_not_in_netbox.length > 0 && (
        <p className="text-data text-sm">
          {data.grafana_not_in_netbox.length} Grafana sites not in NetBox
        </p>
      )}
      {data.netbox_not_in_grafana.length > 0 && (
        <p className="text-data text-sm">
          {data.netbox_not_in_grafana.length} NetBox circuits with no Grafana
          match
        </p>
      )}
      {data.zero_incident_sites.length > 0 && (
        <p className="text-data text-sm text-yellow">
          {data.zero_incident_sites.length} sites with zero incidents in 90 days
        </p>
      )}
      {data.snow_orphan_codes.length > 0 && (
        <p className="text-data text-sm">
          {data.snow_orphan_codes.length} ServiceNow codes not in Grafana
        </p>
      )}
    </div>
  );
...
export default function Dashboard() {
  ...
  const detectionPct =
    stats?.pct_with_precursor != null
      ? Math.round(stats.pct_with_precursor)
      : null;
    ...
    >
        ...
        <>
          <div style={{ gridColumn: 'span 3' }}>
            <p style={{ fontSize: '14px', color: 'var(--color-text-muted)' }}>
              Conflict activity above normal levels — network disruption
              possible within 24-72h
            </p>
          </div>
          <div style={{ gridColumn: 'span 3' }}>
      ...
      >
        ...
        <p className="text-small text-text-muted mt-1">
          AI-generated briefing based on the latest data — coming soon
        </p>
      ...
      >
        <div className="px-5 pt-5 pb-3">
          ...
          <p className="text-small text-text-muted mt-1">
            Combined risk score from conflict data, outage monitoring, and
            internal network alerts
          </p>
        </div>
        {countriesData?.countries && (
          <RiskTable countries={countriesData.countries} />
        )}
      </div>
      ...
      >
        ...
        <p className="text-small text-text-muted mt-1 mb-3">
          Monthly incident distribution across ICT regions — each dot represents
          ~50 incidents
        </p>
        <DotHistogram />
      </div>
      ...
      >
        ...
        <p className="text-small text-text-muted mt-1 mb-3">
          Field sites ranked by total alert volume in the current dataset
        </p>
          ...
          <ol className="space-y-1.5">
            {topDelegations.map((d, i) => (
              <li key={d.site_code} className="flex items-center gap-3">
                <span className="text-data text-text-muted w-5 text-right shrink-0">
                  {i + 1}.
                </span>
                <span className="text-data text-text-primary w-12 shrink-0">
                  {d.site_code}
                </span>
                <span className="text-data text-text-primary w-10 shrink-0">
                  {d.incident_count_30d}
                </span>
      ...
      >
        <SourceHealth
          ...
          lastRun={pipeline_status.last_run}
        />
      </div>
